from image_tools import create_split, convert_label_to_image, return_untrained_images
import os
...
from tensorflow.keras.layers import Input, Conv2D, MaxPooling2D, UpSampling2D, concatenate, Cropping2D, Dropout, Conv2DTranspose
from keras.utils import to_categorical
from tensorflow.keras.callbacks import Callback
import matplotlib.pyplot as plt
import seaborn as sns
from tensorflow.keras.metrics import IoU, MeanIoU, OneHotMeanIoU, OneHotIoU
...
def Unet(input_size=(1512, 2016, 3), num_classes=num_classes, num_filters_multiplier=4):
    ...
    # Concatenates upsampled features with the corresponding encoder features to retain high resolution features (REWRITE)
...
def final_evaluation(model_paths, results_dir="test_results"):
    print("Loading Images")
    new_test_image_paths, new_test_label_paths = return_untrained_images()
    image_test_set = load_images(new_test_image_paths[100:111])
    label_test_set = load_labels(new_test_label_paths[100:111])
    label_test_set_cat = to_categorical(label_test_set, num_classes=num_classes)
    ...
    for path in model_paths:
        ...
        # Evaluate the model
        print("Running Eval")
        results = model.evaluate(image_test_set, label_test_set_cat, return_dict=True)
...
def load_custom_model(path):
    ...
    # Explicitly specify the optimizer and include metrics and the correct loss function
